...
class SPCEStrategy(AbstractStrategy):
    ...
    从实际经验来看，实际情况跟理想情况的差别是有的，比如交易价格跟开盘和收盘价格有比较大的偏差，特别是在开盘的时候，因为开盘的时候经常会有比较大的波动，
    再者由于决策时间点不是严格的开盘和收盘时刻，所以交易信号也存在误差。可以尝试下面几种议价算法
        1 市价单， 市价单基本上是马上成交的，使用这种议价算法的话，可以在下单时间上比较精确的匹配开盘和收盘时刻
        2 中间价挂单，尝试使用买卖价的中间价挂单，并且在价格向一个方向快速变动时，快速调整为新的中间价，这可能会导致成交价距离目标价很远
        3 波动率约束下的中间价挂单。
